...
7. The tray icon menu can show the palette, hide it, open settings, reload prompt JSON, and exit.
...
- The button manager UI is deferred.
- Advanced paste settings are deferred.
- Full no-activate paste behavior must be tested manually against target applications.
- `NumPad` hotkeys require the key to be reported as a numpad virtual key; with NumLock off, Windows may report the physical key as a navigation key such as `End`.
